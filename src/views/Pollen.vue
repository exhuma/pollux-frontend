...
export default {
  ...
  methods: {
    ...
    filterGenera: function (item, queryText) {
      if (queryText === "") {
        return true;
      }
  ...
  mounted() {
    ...
    this.$refs.timelineRef.on("plotly_relayout", function (evt) {
      ...
      self.proxy.getBetween(self.genus, from, to).then((data) => {
        this.timelineData = data;
        Plotly.react("Timeline", [this.timelineData], this.timelineLayout);
      });
    });
